...
export default {
  created() {
    ...
    if(PreVuexStoreState){
      this.$store.replaceState(Object.assign({},this.$store.state,JSON.parse(PreVuexStoreState)))
      // 移除session
    ...
    window.addEventListener('beforeunload',()=>{
      sessionStorage.setItem('PreVuexStoreState',JSON.stringify(this.$store.state))
    })
  },
}
</script>
