    ...
    <transition name="el-zoom-in-top">
      <el-card v-show="showEnvironmentalPrediction">
        ...
        >
          ...
          <el-col :span="2">
            <el-button type="primary" @click="SwitchingFunction"
              >数据查询</el-button
            >
          </el-col>
        </el-row>
                    ...
                    <el-row>
                      ...
                      <el-col :span="2">
                        <el-button
                          type="primary"
                          size="mini"
                          :disabled="showfrom == false ? false : true"
                          @click="closeforecast"
                          >关闭</el-button
                        >
                      </el-col>
...
export default {
  ...
  methods: {
    ...
    async getForecastData(forecaseForm) {
      ...
      let Unit = "";
      let arrUnitList =
        forecaseForm.typeId === "0"
          ? this.weatherCheckItemName
          : this.pondCheckItemName;
      Unit = arrUnitList.find(
        (item) => item.value === forecaseForm.checkItemName
      );
      Unit = Unit.label;
      ...
      this.warn =
        res.data.warn === true
          ? "预测到接下来一段时间内，当前通道值可能会超出或低于阀值，请及时做出相应处理"
          : "无";
      const length = this.getTimeLength(
        forecaseForm.startTime,
        forecaseForm.endTime
      );
